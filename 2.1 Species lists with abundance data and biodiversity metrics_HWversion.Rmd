...
MH=read.csv("Wlikia2.csv")
MH
head(MH)
tail(MH)
MH_list <- MH_list[7]
MH_list <- count(MH_list)
...
head(POM)
tail(POM)
speciesID=paste(POM$Scientific, POM$Name, sep=" ")
speciesID
POM_list=count(speciesID)
head(POM_list)
POM_list=data.frame("POM", POM_list)
names(POM_list)<-c("ListID", "SpeciesID", "Freq") 

#combining both datas into one
Welikia2=rbind(POM_list, MH_list)
dim(Welikia2)
#And run the matrify function
wab=matrify(Welikia2)
wab
...
#then we can compute 
simpson2<-diversity(wab, index = "simpson")
shannon2<-diversity(wab, index = "shannon")
simpson2
shannon2
jaccard2 = vegdist(wab, "jaccard")
jaccard2
bray2 = vegdist(wab, "bray")
bray2 
